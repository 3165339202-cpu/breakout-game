...
void InitChineseFont() {
    const char* text = "分数生命暂停继续重新开始游戏结束胜利排行榜第名按P-暂停按R-重新开始时间倍率落地惩罚恭喜进入空格发射等待";
...
int main() {
    ...
    if (brick.IsActive() && ball.CheckBrickCollision(brick.GetRect())) {
        ...
        Rectangle hitRect = brick.GetRect();

        brick.SetActive(false);
        score += CalculateScore(10, gameTime);
        if (type == GOLDEN) {
        score += CalculateScore(50, gameTime);  // 金砖加 50 分
        // 可选: 播放音效
       // PlaySound(goldSound);
        } else {
        score += CalculateScore(10, gameTime);  // 普通砖
        }
        winCount--;
        ...
        if (type == EXPLOSIVE) {
            for (auto& other : bricks) {
                ...
                if (fabs(r.x - hitRect.x) <= 100 &&
                    fabs(r.y - hitRect.y) <= 40) {
